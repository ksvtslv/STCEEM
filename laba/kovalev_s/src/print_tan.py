...
def line_coef_by_two_points(x, y):
    return ((y[1]-y[0])/(x[1]-x[0]),(y[0]*x[1]-x[0]*y[1])/(x[1]-x[0]))
...
def plot_tangents():
    ...
    to_plot = ypoints
    plt.plot(np.arange(0, to_plot.size), to_plot)
    for i in range(30,40):
        k,b = line_coef_by_two_points([indexes[0]-i-1, indexes[0]-i], [ypoints[indexes[0]-i-1], ypoints[indexes[0]-i]])
        plt.plot([indexes[0]-i-1, indexes[0]-i], [ypoints[indexes[0]-i-1], ypoints[indexes[0]-i]], 'ro')
        plt.plot(np.arange(0, to_plot.size), to_plot)
        plt.plot(np.arange(indexes[0]-i-1, indexes[3]),k*np.arange(indexes[0]-i-1, indexes[3])+b)
        plt.show()
    #plt.plot(indexes, to_plot[indexes], 'ro')
    #plt.plot(minus_indexes, to_plot[minus_indexes], 'go')
